...
function renderGame() {
    ...
    for (var i = 0; i < gameobjects.level.groundcontainer.length; i++) {
        ...
        context.drawImage(img.road, element.x, element.y - 20, element.width, element.height + 20);
    }
    (function() {
        gameobjects.player.animationframe += 0.2;
        if (gameobjects.player.animationframe > gameobjects.player.framelength) gameobjects.player.animationframe = 0;
        var frame = [Math.floor(gameobjects.player.animationframe)]; //debug player collision
        context.drawImage(img.pixel, gameobjects.player.x, gameobjects.player.y, gameobjects.player.width, gameobjects.player.height);
        context.drawImage(img.playerrun, gameobjects.player.tilex * frame, 0, gameobjects.player.tilex, gameobjects.player.tiley,
...
function renderUI() {
    context.drawImage(img.background, 0, 0, GUI.canvas.width, GUI.canvas.height);
    ...
    context.drawImage(img.bg, gameobjects.background.x, gameobjects.background.y, gameobjects.background.width, gameobjects.background.height);
    context.drawImage(img.bg, gameobjects.background.x + gameobjects.background.width, gameobjects.background.y, gameobjects.background.width, gameobjects.background.height);
    ...
    {
        renderGame();
        spritefont(spritefont1, 'SCORE:' + gameobjects.player.score + '  HEALTH:' + gameobjects.player.health, 60, spacing, 800, 100, true); //memory leak debugger ===========================================================================
    ...
    } else if (gameobjects.gamestate == gameobjects.states.info) // info
    {
        ...
        context.drawImage(img.cart, 1050, 430, 150, 100);
        spritefont(spritefont1, 'DODGE ENEMIES                    ', 60, spacing, 800, 560, true);
        context.drawImage(img.button, 750, 560, 50, 50);
        context.drawImage(img.enemy50, 820, 550, 100, 100);
        context.drawImage(img.seel, 930, 550, 100, 100);
        context.drawImage(img.crow4, 1050, 550, 150, 100);
        spritefont(spritefont1, 'PICK COLLECTABLES       TO GAIN HEALTH', 60, spacing, 800, 670, true);
        context.drawImage(img.healthpickup, 800, 660, 70, 70);
        spritefont(spritefont1, 'PRESS OK BUTTON TO GO BACK TO MAIN MENU', 80, spacing, 800, 800, true);
