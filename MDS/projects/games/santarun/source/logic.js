                    ...
                    if (gameobjects.level.enemycontainer[j].damageble) {
                        if (checkIntersection(gameobjects.level.bulletcontainer[i], gameobjects.level.enemycontainer[j])) {
                            ...
                            if (gameobjects.level.enemycontainer[j].health < 1) {
                                if (random(1, 5) == 1) collectale(gameobjects.level.enemycontainer[j]);
...
} //====================================== Referenced functions( GAME SPECIFIC ) ===================================================
...
function collectale(t) {
    var type = random(1, 10);
    if (type == 2 && gameobjects.player.health < gameobjects.defaults.playerHealth * 2 - gameobjects.defaults.healthpickup)
        gameobjects.level.collectablecontainer.push(new collectableConstructor(t, 2));
    if (type == 1 && gameobjects.player.ammo < gameobjects.defaults.ammo * 2 - gameobjects.defaults.ammopickup)
        gameobjects.level.collectablecontainer.push(new collectableConstructor(t, 1));
